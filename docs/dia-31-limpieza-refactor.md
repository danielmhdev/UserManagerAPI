...
| `user.routes.ts`     | Define los _endpoints_ (rutas) REST de la entidad usuario y los vincula con su controlador. |
...
- **En la base de datos:** Se almacena en la columna `passwordHash` de la tabla `User` en PostgreSQL.
- **En el código:** Se genera dentro de la capa de servicio (`user.service.ts` / antes en el controlador) como una cadena de texto simulada (`hash_temporal_${cleanPassword}`) antes de enviarse a `prisma.user.create()` a través del repositorio.

---

### 2. ¿Qué funciones se verán afectadas por `bcrypt`?

- **`createUser` / `registerUser` (Servicio):** Deberá usar `await bcrypt.hash(password, 10)` para encriptar la contraseña antes de persistirla.
- **`loginUser` (Servicio de autenticación):** Deberá usar `await bcrypt.compare(password, user.passwordHash)` para validar las credenciales entrantes.
- **`updatePassword` (Servicio):** Deberá hashear la nueva contraseña antes de actualizar el registro.
- **`seed.ts`:** El script de datos iniciales deberá generar contraseñas con hash real de bcrypt para permitir el inicio de sesión de prueba.
