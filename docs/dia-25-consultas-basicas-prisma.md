...
`server.ts` está acumulando múltiples responsabilidades que deberían estar desacopladas (_violación del Principio de Responsabilidad Única_):
...
- **Conflictos en Git (_Merge Conflicts_):** Varios desarrolladores modificando el mismo archivo central simultáneamente generarán bloqueos continuos.
