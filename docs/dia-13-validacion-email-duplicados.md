...
Usar `409` en lugar de `400` proporciona mucha más semántica a la API. De este modo, el cliente o el frontend pueden identificar exactamente que el fallo se debe a un duplicado y mostrar un mensaje específico al usuario (como _"Este correo ya está en uso, prueba a iniciar sesión"_), en lugar de tratarlo como un error genérico de entrada de datos.
